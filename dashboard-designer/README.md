...
python3 shared/scripts/dependency_checker.py --check
```

### 依赖管理

插件首次使用时会自动检查依赖，如果缺失会提示安装：
...
# 一键安装缺失依赖
python3 shared/scripts/dependency_checker.py --install
...
| 依赖包 | 用途 | 影响功能 |
|--------|------|----------|
| python-docx | Word文档解析 | requirement-analyzer |
| openpyxl | Excel文件生成 | requirement-analyzer |
| PyPDF2 | PDF文档解析 | requirement-analyzer |
| pyyaml | YAML配置解析 | config_manager |
...
从 v2.0 开始，插件支持用户指定独立的工作目录，实现多项目隔离管理：
...
首次使用时，插件会询问用户指定或创建工作目录。

### 项目管理命令

```bash
# 初始化工作目录
python3 shared/scripts/config_manager.py init --path /path/to/workspace

# 创建新项目
python3 shared/scripts/config_manager.py create-project --workspace /path/to/workspace --name "销售驾驶舱"

# 列出所有项目
python3 shared/scripts/config_manager.py list-projects --workspace /path/to/workspace
```
